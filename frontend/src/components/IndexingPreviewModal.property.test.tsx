    ...
    it('all files within excluded directory are hidden', async () => {
      await fc.assert(
        fc.asyncProperty(
          ...
          fc.array(
            fc.stringMatching(/^[a-z][a-z0-9]*\.[a-z]+$/),
          ...
          async (dirNames, fileNames) => {
            ...
            const files = fileNames.map((name, i) => 
              `${directories[i % directories.length]}/${name}`
            );
      ...
      await fc.assert(
        fc.asyncProperty(
          ...
          async (dirNames, fileNames) => {
            ...
            const initialVisibleFiles = files.filter(file => 
              screen.queryByText(file) !== null
            );
            ...
            if (checkbox) {
              ...
              const finalVisibleFiles = files.filter(file => 
                screen.queryByText(file) !== null
              );
      ...
      await fc.assert(
        fc.asyncProperty(
          ...
          async (dirNames, fileNames, dirExcludeIdx, fileExcludeIdx) => {
            ...
            const dirCountText = expectedDirCount === 1 ? '1 directory' : `${expectedDirCount} directories`;
