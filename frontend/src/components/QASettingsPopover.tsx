...
export function QASettingsPopover({ settings, onChange }: QASettingsPopoverProps) {
  ...
  const temperatureLabel =
    settings.temperature <= 0.3 ? 'Precise' : settings.temperature <= 0.6 ? 'Balanced' : 'Creative'
        ...
        <div className="absolute right-0 bottom-full mb-2 w-64 bg-white dark:bg-gray-800 rounded-lg shadow-lg border border-gray-200 dark:border-gray-700 z-50">
          ...
          <div className="p-3 space-y-4">
            ...
            <div>
              <label className="block text-xs font-medium text-gray-700 dark:text-gray-300 mb-2">
                Temperature{' '}
                <span className="text-gray-500">[{settings.temperature.toFixed(1)}]</span>
              </label>
              <input
                type="range"
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) =>
                  onChange({ ...settings, timeoutMinutes: parseInt(e.target.value) })
                }
                className="w-full"
                aria-label="Timeout"
              />
