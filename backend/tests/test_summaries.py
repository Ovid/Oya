...
def yaml_file_summary_markdown_strategy(draw):
    ...
    purpose = draw(
        st.text(alphabet=YAML_SAFE_ALPHABET, min_size=1, max_size=100).filter(lambda x: x.strip())
    )
    layer = draw(st.sampled_from(VALID_LAYERS))
    key_abstractions = draw(
        st.lists(
    ...
    markdown_content = draw(
        st.text(alphabet=YAML_SAFE_ALPHABET + "#*[]()!", min_size=1, max_size=200).filter(
            lambda x: x.strip()
        )
    )
...
class TestYAMLParsingAndStripping:
    ...
    def test_file_summary_yaml_extraction_and_stripping(self, data):
        ...
        clean_markdown, summary = parser.parse_file_summary(
            data["markdown"], file_path="test/file.py"
        )
    ...
    def test_directory_summary_yaml_extraction_and_stripping(self, data):
        ...
        clean_markdown, summary = parser.parse_directory_summary(
            data["markdown"], directory_path="test/dir"
        )
...
class TestFileSummaryFallbackOnParseFailure:
    ...
    @given(
        file_path=st.text(
            alphabet="abcdefghijklmnopqrstuvwxyz0123456789/_-.", min_size=1, max_size=100
        ).filter(lambda x: x.strip())
    )
    @settings(max_examples=100)
...
def synthesis_map_strategy(draw):
    ...
    project_summary = draw(
        st.text(alphabet=YAML_SAFE_ALPHABET, min_size=1, max_size=500).filter(lambda x: x.strip())
    )
...
def file_summaries_for_layer_grouping_strategy(draw):
    ...
    for i in range(num_files):
        ...
        purpose = draw(
            st.text(alphabet=YAML_SAFE_ALPHABET, min_size=1, max_size=100).filter(
                lambda x: x.strip()
            )
        )
        layer = draw(st.sampled_from(VALID_LAYERS))
...
class TestSynthesisMapLayerGroupingCompleteness:
    ...
    def test_only_valid_layers_in_synthesis_map(self, file_summaries_data):
        ...
        for layer_name in synthesis_map.layers.keys():
            assert layer_name in VALID_LAYERS, f"Invalid layer name {layer_name} in synthesis map"
...
def large_file_summaries_strategy(draw):
    ...
    for i in range(num_files):
        ...
        purpose = draw(
            st.text(alphabet=YAML_SAFE_ALPHABET, min_size=100, max_size=500).filter(
                lambda x: x.strip()
            )
        )
        ...
        internal_deps = draw(
            st.lists(
                st.text(
                    alphabet="abcdefghijklmnopqrstuvwxyz0123456789/_-.", min_size=10, max_size=60
                ).filter(lambda x: x.strip()),
                min_size=1,
                max_size=8,
            )
        ...
        external_deps = draw(
            st.lists(
                st.text(
                    alphabet="abcdefghijklmnopqrstuvwxyz0123456789_-", min_size=3, max_size=30
                ).filter(lambda x: x.strip()),
                min_size=1,
                max_size=5,
            )
...
class TestSynthesisBatchingForLargeInputs:
    ...
    def test_batching_produces_valid_synthesis_map(self, file_summaries_data):
        ...
        batches = generator.create_batches(
            file_summaries, directory_summaries, context_limit=context_limit
        )
        ...
        for i, batch in enumerate(batches):
            ...
            batch_tokens = generator.estimate_token_count(batch_file_summaries, batch_dir_summaries)
        ...
        assert all_files_in_batches == original_files, "All files should be covered across batches"
    ...
    def test_batching_with_large_context_creates_single_batch(self, file_summaries_data):
        ...
        assert len(batches) == 1, f"Expected 1 batch with large context limit, got {len(batches)}"
    ...
    def test_merged_synthesis_map_contains_all_layers(self, file_summaries_data):
        ...
        original_files = {fs.file_path for fs in file_summaries}
        assert all_files_in_merged == original_files, "Merged map should contain all original files"
...
class TestSynthesisMapExtended:
    ...
    def test_synthesis_map_json_roundtrip_with_new_fields(self):
        ...
        original = SynthesisMap(
            layers={
                "api": LayerInfo(name="api", purpose="HTTP handlers", directories=[], files=[])
            },
            key_components=[],
            dependency_graph={},
            ...
            entry_points=[
                EntryPointInfo(
                    name="serve", entry_type="main_function", file="app.py", description=""
                )
            ],
