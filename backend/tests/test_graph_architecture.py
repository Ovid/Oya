...
def sample_graph():
    ...
    G.add_node(
        "llm/client.py::generate",
        name="generate",
        type="function",
        file_path="llm/client.py",
        line_start=1,
        line_end=50,
        docstring="Generate LLM response",
    )
    G.add_edge(
        "api/routes.py::handle", "db/queries.py::get_user", type="calls", confidence=0.9, line=15
    )
    G.add_edge(
        "api/routes.py::handle", "llm/client.py::generate", type="calls", confidence=0.85, line=20
    )
    return G
