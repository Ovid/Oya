...
class SynthesisGenerator:
    ...
    async def generate(
        ...
        if self.llm_client is not None:
            ...
            if total_tokens > self.context_limit:
                ...
                # Process all at once
                synthesis_map = await self._process_batch(
                    file_summaries, 
                    directory_summaries
                )
        ...
        try:
            ...
            for comp in data.get("key_components", []):
                if isinstance(comp, dict):
                    key_components.append(ComponentInfo(
                        name=comp.get("name", ""),
                        file=comp.get("file", ""),
                        role=comp.get("role", ""),
    ...
    def create_batches(
        ...
        for ds in directory_summaries:
            ...
            if current_tokens + ds_tokens > context_limit and (current_file_batch or current_dir_batch):
                # Start a new batch
                batches.append((current_file_batch, current_dir_batch))
                current_file_batch = []
                current_dir_batch = []
